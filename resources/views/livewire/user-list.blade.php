            ...
            <tbody class="divide-y divide-gray-200">
                @foreach ($users as $user)
                    <tr>
                        ...
                        <td>    
                            <flux:button variant="danger" wire:click="deleteUser({{ $user->id }})" wire:confirm.prompt="本当に削除しますか？\n\n削除する場合はDELETEと入力してください|DELETE">削除</flux:button>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            @foreach ($roles as $role)
                                ...
                                <span wire:click="toggleRole({{ $user->id }}, {{ $role->id }})" class="inline-block cursor-pointer px-2 py-1 rounded mr-1 {{ $assigned ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-800' }}">
                                    {{ $role->name }}
                                </span>
                            @endforeach
                        </td>
